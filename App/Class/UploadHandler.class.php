...
class UploadHandler
{
    ...
    function __construct($dir,$type,$name,$options = null, $initialize = true, $error_messages = null) {
        $this->name=$name;
        $this->type=$type;
        $this->options = array(
    ...
    protected function change_file_name($content){
        // 改变文件名
        $a=time();
        $this->file_name='App/Upload/'.$this->type.'/'.$this->name.'/'.$a.".jpg";
        $old_name='App/Upload/'.$this->type.'/'.$this->name.'/'.$content['files'][0]->name;
        // $file_url=$content['files'][0]->url;
        // $file_thumbnailUrl=$content['files'][0]->thumbnailUrl;
        // $file=fopen($file_url, 'w+');
       rename($old_name,$this->file_name);
        $this->file_name='App\/Upload\/'.$this->type.'\/'.$this->name.'\/'.$a.".jpg";
    }
